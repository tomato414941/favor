...
export class LocalAuth {
  ...
  async login(input: LocalCredentials, previousSession?: string): Promise<string> {
    const email = normalize(input.email);
    if (!emailValid(email) || !passwordValid(input.password)) throw new DomainError('UNAUTHORIZED', loginError, 401);
    this.auth.limit(`password:${email}`, 10);
    const row = this.auth.store.db.prepare('SELECT * FROM local_credentials WHERE login = ?').get(email);
    ...
    if (!timingSafeEqual(actual, expected) || !row) throw new DomainError('UNAUTHORIZED', loginError, 401);
    return this.auth.store.transaction(() => {
