...
if (!process.argv.includes('--demo')) throw new Error('Run with --demo to use the mock-payment application.');
...
if (!['local', 'demo', 'x'].includes(authMode)) throw new Error('COMMISSION_AUTH_MODE must be local, demo or x.');
const publicOrigin = process.env.COMMISSION_PUBLIC_ORIGIN;
const trustProxy = process.env.COMMISSION_TRUST_PROXY ?? 'none';
if (!['none', 'loopback'].includes(trustProxy)) throw new Error('COMMISSION_TRUST_PROXY must be none or loopback.');
const xProvider = authMode === 'x' ? new XProvider({
  clientId: process.env.X_CLIENT_ID ?? '', clientSecret: process.env.X_CLIENT_SECRET ?? '',
  publicOrigin: publicOrigin ?? '', appBearerToken: process.env.X_APP_BEARER_TOKEN,
}) : undefined;
...
const app = await buildApp(service, { logger: true, demoAuth: authMode === 'demo', localAuth: true, xProvider,
  publicOrigin, trustLoopbackProxy: trustProxy === 'loopback' });
const timer = setInterval(() => {
  try { service.expire(); } catch { app.log.error('Request expiration failed'); }
}, 1000);
timer.unref();
let closing = false;
