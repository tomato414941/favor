...
export class XProvider {
  ...
  constructor(
    ...
    if (!config.clientId.trim() || !config.clientSecret.trim())
      throw new Error('X_CLIENT_ID and X_CLIENT_SECRET are required in X authentication mode.');
  ...
  async authenticate(code: string, verifier: string): Promise<SocialAccount> {
    ...
    const token = await this.json('https://api.x.com/2/oauth2/token', {
      method: 'POST',
      headers: {
    ...
    if (
      ...
      !X_SCOPES.split(' ').every((scope) => (token.scope as string).split(' ').includes(scope))
    )
      throw unavailable();
    // No offline scope, refresh token or user access token is saved or returned to the browser.
    return this.account(
      await this.json('https://api.x.com/2/users/me', {
...
export class XAuth {
  ...
  start(ip: string, previousBrowser: string | undefined, previousSession: string | undefined) {
    ...
    this.auth.store.transaction(() => {
      ...
      if (
        Number(
          this.auth.store.db.prepare('SELECT COUNT(*) AS total FROM oauth_flows').get()!.total,
        ) >= 1000
      )
        throw unavailable();
      this.auth.store.db
        ...
        .run(
          ...
          hashToken(browser),
          verifier,
          isToken(previousSession) ? hashToken(previousSession) : null,
          this.auth.clock() + FLOW_MS,
        );
    });
    return {
      browser,
  ...
  cancel(browser: string | undefined) {
    if (isToken(browser))
      this.auth.store.db
        .prepare('DELETE FROM oauth_flows WHERE browser_hash = ?')
        .run(hashToken(browser));
  }
  async finish(browser: string | undefined, query: Record<string, unknown>): Promise<string> {
    ...
    const row = this.auth.store.transaction(() => {
      const flow = this.auth.store.db
        .prepare(
          "SELECT * FROM oauth_flows WHERE state_hash = ? AND browser_hash = ? AND expires_at > ? AND verifier != ''",
        )
        .get(hashToken(state), hashToken(browser), this.auth.clock());
      if (!flow) throw invalidFlow();
      this.auth.store.db
        .prepare("UPDATE oauth_flows SET verifier = '' WHERE state_hash = ?")
        .run(hashToken(state));
      return flow;
    });
    try {
      if (query.error === 'access_denied')
        throw new DomainError('OAUTH_CANCELLED', 'Xでの確認を中止しました。', 400);
      if (
      ...
      return this.auth.store.transaction(() => {
        ...
        if (
          !this.auth.store.db
            .prepare(
              'SELECT 1 FROM oauth_flows WHERE state_hash = ? AND browser_hash = ? AND expires_at > ?',
            )
            .get(hashToken(state), hashToken(browser), this.auth.clock())
        )
          throw invalidFlow();
        return this.auth.xLogin(
          account,
          row.previous_session_hash === null ? null : String(row.previous_session_hash),
        );
      });
    } finally {
      this.auth.store.db
