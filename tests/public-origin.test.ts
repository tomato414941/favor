...
test('公開URLで登録・ログイン・ログアウトし、HTTPS専用のCookieと送信元の確認を適用する', async () => {
  ...
  try {
    ...
    for (const changedHeaders of [
      ...
      assert.equal((await app.inject({ method: 'POST', url: '/api/auth/local/login', headers: changedHeaders, payload: credentials })).statusCode, 403);
    }
...
test('公開URLにHTTPSを要求し、固定アカウントの体験モードをループバックに限定する', async () => {
  ...
  try {
    for (const value of ['invalid', 'http://commission.example', `${publicOrigin}/`, `${publicOrigin}/path`, 'https://user:pass@commission.example']) {
      await assert.rejects(buildApp(service, { localAuth: true, publicOrigin: value }), /COMMISSION_PUBLIC_ORIGIN/);
    }
    await assert.rejects(buildApp(service, { demoAuth: true, publicOrigin }), /only on loopback/);
    await assert.rejects(buildApp(service, { trustLoopbackProxy: true }), /COMMISSION_PUBLIC_ORIGIN/);
    const app = await buildApp(service, { demoAuth: true, publicOrigin: 'http://127.0.0.1:3211' });
    try {
      assert.equal((await app.inject({ url: '/api/health', headers: { host: '127.0.0.1:3211' } })).statusCode, 200);
...
test('同じ端末の認証試行を制限し、信頼するプロキシ経由の別端末には試行を許可する', async () => {
  ...
  try {
    ...
    for (let i = 0; i < 30; i++) assert.equal((await attempt('192.0.2.12', `198.51.100.${i + 1}`)).statusCode, 401);
